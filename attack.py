...
from attacker.prompt import PromptAttacker
...
class DGAttackEval(DGDataset):
    def __init__(
        ...
        combined = "combined" if args.use_combined_loss and att_method == 'prompt' else "single"
        max_per = args.max_per
        fitness = args.fitness if att_method == 'prompt' else 'performance'
        select_beams = args.select_beams if att_method == 'prompt' else 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_beams", type=int, default=2, help="Number of beams for decoding in LLMs")
    ...
    parser.add_argument("--model_name_or_path", "-m", type=str, default="Microsoft/DialoGPT-medium", help="Path to model")
