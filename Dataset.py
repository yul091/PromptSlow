...
class DGDataset:
    def __init__(
        ...
        task: str = "seq2seq",
        tokenizer: Union[BertTokenizer, GPT2Tokenizer] = None,
    ...
    def prepare_context(self, instance: dict):
        if self.dataset == 'blended_skill_talk':
            ...
            if self.task == 'seq2seq':
                persona_pieces = f"(Persona: {instance['personas'][1]})"
                if instance['context'] == "wizard_of_wikipedia":
                    additional_context_pieces = f"(Context: {instance['additional_context']})"
                else:
            ...
            if self.task == 'seq2seq':
                ...
                persona_pieces = f"(Persona: {user_profile})"
            ...
            num_entries = total_entries//2
            if self.task == 'seq2seq':
                persona_pieces = f"(Persona: {instance['prompt']})"
                additional_context_pieces = f"(Context: {instance['context']})"
                context = persona_pieces + additional_context_pieces
            else:
                num_entries = min(num_entries, 2)
                context = ''
